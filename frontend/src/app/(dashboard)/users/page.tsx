...
export default function UsersPage() {
  ...
  const closeModal = () => {
    ...
    setEditingUser(null);
    setFormValues({ name: '', email: '', role: 'TECHNICIAN', password: '' });
  };
  ...
  const openEditModal = (item: EditableUser) => {
    setEditingUser(item);
    setFormValues({ name: item.name, email: item.email, role: item.role, password: '' });
    setUserModalOpen(true);
  };
  ...
  if (!canManageUsers) {
    return (
      <Alert variant="light" color="yellow" title="Nemáte oprávnění">
        <Stack gap="xs">
          <Text size="sm">Spravovat uživatele může pouze administrátor.</Text>
          <Button component={Link} href="/" leftSection={<IconArrowLeft size={16} />} size="xs">
            Zpět na přehled
          </Button>
        </Stack>
      </Alert>
  ...
  return (
    <Stack gap="xl">
      <Group justify="space-between" align="center">
        <div>
          <Title order={2}>Správa uživatelů</Title>
          <Text c="dimmed" size="sm">
            Přidávejte nové uživatele a nastavujte jejich role (Admin, Účetní, Technik).
          </Text>
        </div>
                  ...
                  <Table.Tr key={item.id}>
                    ...
                    <Table.Td ta="right">
                      <Group gap="xs" justify="flex-end">
                        <ActionIcon variant="subtle" onClick={() => openEditModal(item)} aria-label="Upravit">
                          <IconPencil size={16} />
                        </ActionIcon>
                        <ActionIcon
                          variant="light"
      ...
      >
        <Stack gap="sm">
          ...
          <Select
            ...
            onChange={(value) => setFormValues((prev) => ({ ...prev, role: value ?? prev.role }))}
            required
          />
          <PasswordInput
