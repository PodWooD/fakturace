...
const parseMockInvoice = (reason = 'OCR nedostupné, použita mock data'): OcrResult => {
    const now = new Date();
    return {
...
const normalizeItems = (items: any[] = []): OcrItem[] => {
...
const mapCurrency = (value: string | undefined) => {
    if (!value) return 'CZK';
    const normalized = value
        .trim()
        .normalize('NFKD')
...
const attemptLocalPdfParse = async ({
    buffer,
    filename,
    mimetype,
}: {
    ...
    try {
        ...
        const currency = extractFirstMatch(joined, [/\b(CZK|Kč|EUR|USD|GBP|PLN)\b/i]) || 'CZK';
        ...
        const supplierLine =
            linesWithPlain.find(
                (entry) => entry.plain.startsWith('dodavatel') || entry.plain.startsWith('supplier')
            ) ||
            linesWithPlain.find(
                (entry) => entry.plain.includes('dodavatel') || entry.plain.includes('supplier')
            );
...
const parseInvoiceFromPages = (pages: any[] = []): OcrResult | null => {
    ...
    let totalWithVat = 0;
    let lastItem: OcrItem | null = null;
    ...
    if (tableMatch) {
        ...
        for (const row of rows) {
            ...
            const referenceCode = extractReferenceProductCode(
                item.itemName || '',
                item.description || ''
            );
            if (referenceCode) {
...
const normalizeResponse = (payload: any): OcrResult => {
    if (!payload) {
    ...
    const base: OcrResult = {
        ...
        invoiceNumber:
            ...
            `TMP-${Date.now()}`,
        issueDate: toIsoDate(
            invoice.date || invoice.issue_date || payload.issueDate || payload.date
        ),
        currency,
        totalWithoutVat: toNumber(
            totals.excluding_vat || totals.net || invoice.total_ex_tax,
            null
        ),
        totalWithVat: toNumber(
            totals.including_vat || totals.gross || invoice.total_inc_tax,
            null
        ),
        items,
    };
...
async function callMistralOcr({
    ...
    try {
        ...
        // @ts-ignore - native fetch vs node-fetch types mismatch
        uploadForm.append('file', blob as any, filename || 'invoice.pdf');
        ...
        if (!signedUrlResponse.ok) {
            ...
            throw new Error(
                `Mistral files url API vrátilo chybu ${signedUrlResponse.status}: ${text}`
            );
        }
        ...
        console.info('[OCR] Mistral OCR úspěšně odpověděl', {
            ...
            items: Array.isArray(responsePayload?.items)
                ? responsePayload.items.length
                : undefined,
            pages: Array.isArray(responsePayload?.pages)
                ? responsePayload.pages.length
                : undefined,
        });
        return normalizeResponse(responsePayload);
    } catch (error: any) {
        console.error(
            '[OCR] Chyba při volání Mistral OCR, zkouším lokální fallback:',
            error.message
        );
        const fallback = await attemptLocalPdfParse({ buffer, filename, mimetype });
        if (fallback) {
