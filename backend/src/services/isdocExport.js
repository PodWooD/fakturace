...
const aggregateWork = (workRecords = [], organization = {}) => {
...
class IsdocExport {
  ...
  _buildSupplier(root) {
    ...
    address.ele('cbc:StreetName').txt((process.env.COMPANY_ADDRESS || 'Ulice 123, Praha').split(',')[0] || 'Ulice 123');
  ...
  _buildCustomer(root, organization) {
    ...
    if (organization.address) {
      const [street = '', city = '', postal = ''] = organization.address.split(',').map((part) => part.trim());
  ...
  _appendInvoiceLine(root, index, { text, quantity, unitPrice, vatRate }) {
    ...
    line
      .ele('cbc:InvoicedQuantity', { unitCode: 'H87' })
      .txt(Number(qty).toFixed(2));
    line
      .ele('cbc:LineExtensionAmount', { currencyID: this.currency })
      .txt(toCurrency(total));
    ...
    priceElement
      .ele('cbc:PriceAmount', { currencyID: this.currency })
      .txt(toCurrency(price));

    const taxTotal = line.ele('cac:TaxTotal');
    taxTotal
      .ele('cbc:TaxAmount', { currencyID: this.currency })
      .txt(toCurrency(vatAmount));
  ...
  _collectLines({ services = [], workRecords = [], hardware = [], organization }) {
    const lines = [];
    ...
    hardware.forEach((item, index) => {
      ...
      const unit = item.unitPriceCents ? item.unitPriceCents / 100 : total / quantity || 0;
      lines.push({
  ...
  generateInvoiceXML(invoice, organization, workRecords, services, hardware, totals) {
    ...
    monetary
      ...
      .txt(toCurrency(totalAmount));
    monetary
      .ele('cbc:TaxInclusiveAmount', { currencyID: currency })
      .txt(toCurrency(totalWithVat));

    return Buffer.from(root.end({ prettyPrint: true }), 'utf8');
  }

  async saveInvoiceISDOC(invoice, workRecords, services, hardware, organization, totals) {
    const xmlBuffer = this.generateInvoiceXML(invoice, organization, workRecords, services, hardware, totals);
