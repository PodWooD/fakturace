...
import { PrismaClient, Organization, Service, Prisma } from '@prisma/client';
import authMiddleware, { authorize } from '../middleware/auth';
import { toCents, fromCents } from '../utils/money';
import { logAudit } from '../services/auditLogger';
import { getCompanyByICO, validateICO } from '../services/aresService';
...
const mapOrganization = (organization: Organization & { services?: Service[] }) => ({
    ...organization,
...
const resolveCentsValue = ({ value, cents }: { value?: any; cents?: any }): number | null => {
...
const parseOrganizationPayload = (payload: any) => {
...
router.post('/', authMiddleware, authorize('organizations:write'), async (req: AuthRequest, res: Response) => {
    try {
        if (!req.body?.name) {
...
router.put('/:id', authMiddleware, authorize('organizations:write'), async (req: AuthRequest, res: Response) => {
    try {
        const { id } = req.params;
...
router.delete('/:id', authMiddleware, authorize('organizations:write'), async (req: AuthRequest, res: Response) => {
    try {
        const { id } = req.params;
        ...
        // @ts-ignore
        const hasRecords = Object.values(org._count).some((count: number) => count > 0);
        if (hasRecords) {
...
router.get('/ares/:ico', authMiddleware, async (req: Request, res: Response) => {
    try {
