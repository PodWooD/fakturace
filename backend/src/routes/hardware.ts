...
import { toCents, fromCents, normalizeNumber } from '../utils/money';
...
const ensureSplittableStatus = (status: string) => {
    return (
...
router.post('/split', authMiddleware, authorize('hardware:write'), async (req: AuthRequest, res: Response) => {
    try {
        ...
        const parts: ReceivedInvoiceItem[] = [];
...
router.post('/assign', authMiddleware, authorize('hardware:write'), async (req: AuthRequest, res: Response) => {
    try {
...
const buildHardwarePayload = async (body: any) => {
    ...
    return {
        ...
        status:
            body.status && (HardwareStatus as any)[body.status] ? body.status : HardwareStatus.ASSIGNED,
        month,
        year,
...
router.post('/', authMiddleware, authorize('hardware:write'), async (req: AuthRequest, res: Response) => {
    try {
...
router.put('/:id', authMiddleware, authorize('hardware:write'), async (req: AuthRequest, res: Response) => {
    try {
        const { id } = req.params;
...
router.post('/:id/status', authMiddleware, authorize('hardware:write'), async (req: AuthRequest, res: Response) => {
    try {
        const { id } = req.params;
        const { status } = req.body;

        if (!status || !(HardwareStatus as any)[status]) {
...
router.delete('/:id', authMiddleware, authorize('hardware:write'), async (req: AuthRequest, res: Response) => {
    try {
        const { id } = req.params;
